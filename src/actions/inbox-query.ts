...
export async function getInboxMessagesAction(query: InboxQuery): Promise<InboxQueryResult> {
  ...
  if (error) {
    // Previously swallowed silently into an empty list — indistinguishable
    // from "no messages" in the UI. Logging it means a transient issue
    // (e.g. a PostgREST schema cache not yet reloaded after a manual
    // migration) shows up in server logs instead of just looking like an
    // empty inbox.
    console.error('[getInboxMessagesAction]', error);
